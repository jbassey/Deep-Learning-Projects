...
class ANN(object):
    ...
    def fit(self, X, Y, learning_rate=1e-2, mu=0.99, decay=0.999, reg=1e-3, epochs=10, batch_sz=100, show_fig=False):
        ...
        for M2 in self.hidden_layer_sizes:
            ...
            self.hidden_layers.append(h) #get weight/biases for hidden layers
            M1 = M2
            count += 1
        W, b = init_weight_and_bias(M1, K) #get weight for last hodden to output layer
        self.W = tf.Variable(W.astype(np.float32))
        self.b = tf.Variable(b.astype(np.float32))

        # collect params for later use
        self.params = [self.W, self.b] #keep all weights and biases starting from output layer
    ...
    def forward(self, X):
        Z = X
        for h in self.hidden_layers:
            Z = h.forward(Z) #update neurons via foward pass
        return tf.matmul(Z, self.W) + self.b #output layer
